<template>
  <BaseModal
    ...
    id="modal_history_kode_klasifikasi"
    :showFooter="false"
  >
    <h3 class="text-center mb-1">History Kode Klasifikasi</h3>
    <p class="text-center">Detail History Kode Klasifikasi yang telah disimpan</p>
    ...
    <div class="row mb-2 mt-2">
      ...
      <div class="col-md-6">
       ...
       <div class="col-md-12 mt-2">
        <label class="form-label" for="selected-kode-klasifikasi">Kode Klasifikasi</label>

        <select
          id="selected-kode-klasifikasi"
          name="selected-kode-klasifikasi"
          class="form-select"
          v-model="pk"
        >
          <option value="">Semua Kode Klasifikasi</option>
          <option
            v-for="(item, index) in listKodeKlasifikasi"
            :key="`${index}-${item?.klasifikasi}`"
            :value="item?.id"
          >
            {{ item.klasifikasi }}
          </option>
      ...
      <div class="col-md-12">
        <div class="table-responsive">
          <v-table
            ...
            :data="list_history_kode_klasifikasi.data"
          ...
          >
            ...
            <tbody slot="body" slot-scope="{ displayData }">
              <tr v-if="list_history_kode_klasifikasi.loading">
                <td colspan="2" class="text-center">Loading...</td>
              </tr>

              <tr
                v-else-if="!list_history_kode_klasifikasi.loading && displayData.length"
                      ...
                      >
                        <div class="d-flex flex-column">
                          ...
                          >
                            ...
                            {{
                              documentParsed(item?.before)?.klasifikasi ||
                              documentParsed(item?.after)?.klasifikasi
                            }}
                          </h5>
                          <div class="d-flex">
                            <div>
              ...
              <tr v-else>
                <td colspan="2" class="text-center">
                  Tidak ada History Kode Klasifikasi ditemukan
                </td>
              </tr>
            </tbody>
          </v-table>
        ...
        <CustomPagination
          v-if="
            !list_history_kode_klasifikasi.loading && list_history_kode_klasifikasi.data.length
          "
          :currentDataLength="list_history_kode_klasifikasi.data.length"
          :totalData="meta.total"
        >
          <template #page>
            <smart-pagination
              v-if="list_history_kode_klasifikasi.data"
...
export default {
  name: "ModalHistoryKodeKlasifikasi",
  components: {
    BaseModal,
    BaseButton,
    Spinner,
  ...
  methods: {
    ...
    async getListHistoryKodeKlasifikasi({ limit, offset, from, to, created_by, pk }) {
      try {
        this.list_history_kode_klasifikasi.loading = true;
        const response = await HistoryActivityNetwork.getListHistoryActivity({
          module: "kodeklasifikasi",
          limit,
          offset,
          from,
          to,
        ...
        if (response) {
          this.list_history_kode_klasifikasi.loading = false;
          this.list_history_kode_klasifikasi.data = response?.data?.rows;
          this.meta = {
  ...
  watch: {
    filterHistory() {
      this.getListHistoryKodeKlasifikasi({

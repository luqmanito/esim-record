<template>
  <BaseModal
    ...
    id="modal_history_layanan"
    :showFooter="false"
    :order="3"
  >
    <h3 class="text-center mb-1">History Layanan</h3>
    <p class="text-center">Detail History Layanan yang telah disimpan</p>
    ...
    <div class="row mb-2 mt-2">
      ...
      <div class="col-md-12 mt-2">
        <label class="form-label" for="selected-layanan">Layanan</label>

        <select
          id="selected-layanan"
          name="selected-layanan"
          class="form-select"
          v-model="pk"
        >
          <option value="">Semua Layanan</option>
          <option
            v-for="(item, index) in listLayanan"
            :key="`${index}-${item?.layanan}`"
            :value="item?.id"
          >
            {{ item.layanan }}
          </option>
      ...
      <div class="col-md-12">
        <div class="table-responsive">
          <v-table
            ...
            :data="list_history_layanan.data"
            :pageSize="params.limit"
            ref="tableHistoryLayanan"
          >
            ...
            <tbody slot="body" slot-scope="{ displayData }">
              <tr v-if="list_history_layanan.loading">
                <td colspan="2" class="text-center">Loading...</td>
              </tr>

              <tr
                v-else-if="!list_history_layanan.loading && displayData.length"
                      ...
                      >
                        <div class="d-flex flex-column">
                          ...
                          >
                            ...
                            {{
                              documentParsed(item?.before)?.layanan ||
                              documentParsed(item?.after)?.layanan
                            }}
                          </h5>
                          <div class="d-flex">
                            <div>
              ...
              <tr v-else>
                <td colspan="2" class="text-center">
                  Tidak ada History Layanan ditemukan
                </td>
              </tr>
            </tbody>
          </v-table>
        ...
        <CustomPagination
          v-if="
            !list_history_layanan.loading && list_history_layanan.data.length
          "
          :currentDataLength="list_history_layanan.data.length"
          :totalData="meta.total"
        >
          <template #page>
            <smart-pagination
              v-if="list_history_layanan.data"
...
<script>
import HistoryActivity from "@/network/lib/history-activity";
...
export default {
  ...
  methods: {
    ...
    async getListHistoryLayanan({ limit, offset, from, to, created_by, pk }) {
      try {
        this.list_history_layanan.loading = true;
        const response = await HistoryActivity.getListHistoryActivity({
          module: "layanan",
          limit,
          offset,
          from,
        ...
        if (response) {
          this.list_history_layanan.loading = false;
          this.list_history_layanan.data = response?.data?.rows;
          this.meta = {
